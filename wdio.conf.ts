 ...
 *
 * You should not have to make changes in this config, this config is defaults.
 * Please make any dynamic config changes in the config/*.conf.ts files
 * those files will control any dynamic conf and will be merged with this static default conf.
 */
...
if (envData.config.cross_browser) {
  dynamicConfig = await import(`./config/cross_browser.conf.ts`);
} else {
  dynamicConfig = await import(`./config/default.conf.ts`);
}
...
export const config: Options.Testrunner = Object.assign(
  {},
  {
    ...
    afterStep: function(step, scenario, result, context) {
        logger.log(
            `  ${step.text} -- ${
                result.passed ? colors.green("Passed") : colors.red("Failed")
            }`
        );
    },
    /**
    ...
    afterScenario: function(world, result, context) {
        logger.log(
            colors.bold(
                `Completed Scenario: ${world.pickle.name} -- ${
                    result.passed ? colors.green("Passed") : colors.red("Failed")
                }`
            )
        );
